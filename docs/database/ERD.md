...
- the 26 seeded dental service records are provisional evaluation records and do not by themselves establish production medical readiness;
...
## 6. Proposed Policy, Evidence, and Eligibility Schema
...
Columns: `id`; `public_id`; `provider_id BIGINT FK providers`; `branch_id BIGINT FK branches`; `service_id BIGINT FK services`; `amount DECIMAL(18,2) NOT NULL`; `currency VARCHAR(3) NOT NULL`; `source_reference VARCHAR(255) NULL`; `effective_from TIMESTAMP NOT NULL`; `effective_until TIMESTAMP NULL`; `verified_at TIMESTAMP NULL`; `supersedes_price_id BIGINT NULL self-FK`; `created_at TIMESTAMP NOT NULL`.

**Indexes:** (`provider_id`, `branch_id`, `service_id`, `effective_from`); (`service_id`, `currency`, `effective_from`) supports price-band classification. This is a price fact only, not a payment record.
...
Columns: `id`; `public_id`; `provider_id BIGINT FK providers`; `branch_id BIGINT FK branches`; `service_id BIGINT FK services`; `service_definition_id BIGINT FK service_definitions`; `final_state VARCHAR(32) NOT NULL`; `scientific_grade VARCHAR(24) NULL`; `pricing_class VARCHAR(24) NULL`; `protection_level VARCHAR(24) NULL`; `internal_risk_code VARCHAR(24) NULL`; `confidence_k DECIMAL(10,4) NULL`; `confidence_eu DECIMAL(10,4) NULL`; `controlling_gate VARCHAR(64) NULL`; `snapshot_json JSON NOT NULL`; `snapshot_hash CHAR(64) NOT NULL`; `evaluated_at TIMESTAMP NOT NULL`; `created_at TIMESTAMP NOT NULL`.
...
**Requirements:** FR-CLINICAL-001–002.

Columns: `id`; `public_id`; `case_id BIGINT FK cases`; `version INT UNSIGNED NOT NULL`; `authored_by_user_id BIGINT FK users`; `state VARCHAR(24) NOT NULL`; `service_id BIGINT FK services`; `inclusions_json JSON NOT NULL`; `exclusions_json JSON NOT NULL`; `terms_json JSON NOT NULL`; `content_hash CHAR(64) NOT NULL`; `proposed_at TIMESTAMP NULL`; timestamps.

**Constraints / indexes:** unique (`case_id`, `version`); (`case_id`, `state`); `content_hash`. Accepted historical content becomes immutable.
...
**Constraints:** unique (`treatment_plan_version_id`, `sequence`). Stage price is part of proposed/accepted terms and does not indicate platform payment custody.
...
1. Stable public catalog identities are immutable after creation.
2. Evaluation catalog data is not equivalent to production readiness.
...
2. policy versions and service-activation/fact/price/evidence metadata;
3. eligibility decisions and gate results;
4. appointment slots, bookings, alternatives, booking events;
5. cases, plan versions/stages, accepted snapshots, case stages, follow-ups;
6. financial terms snapshots and append-only financial events;
...
| Q-CATALOG-001 | Major | Production service data cannot be treated as clinically approved from the current 26 evaluation records. |
| Q-ELIG-001 | Major | Production S/P/H/I rule content and thresholds must be licensed/approved before production policy rows are activated. |
