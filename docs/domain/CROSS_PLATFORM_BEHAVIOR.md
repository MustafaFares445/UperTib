...
`Q-PLATFORM-001` still prevents claiming complete reconciliation against readable authoritative SRS v1.1. Production clinical catalog and S/P/H/I behavior remain governed by `Q-CATALOG-001` and `Q-ELIG-001`. Concrete external notification/evidence vendor selection remains unresolved under `Q-OPS-001`; `Q-PLATFORM-003` is Resolved for the provider-neutral contract.
