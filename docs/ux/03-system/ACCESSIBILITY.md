...
| Field | Value |
|---|---|
| Purpose | Distinguish the page-level obligation (no 2D scroll) from the legitimate exception (a data table's own horizontal scroll), which is `A11Y-PLATFORM-036`'s job specifically — this obligation states the page-level rule that exception sits inside. Also distinguish this criterion from `A11Y-PLATFORM-019`'s: SC 1.4.4 (200% text resize, no loss of content or function) and SC 1.4.10 (reflow at the 400%-zoom-equivalent width) are two separate success criteria with two separate thresholds and must not be conflated, per `accessibility/wcag-checklist.md`. |
| Applies to | Both profiles |
| Requirement source | WCAG 2.2 SC 1.4.10 Reflow |
| User impact | A user zoomed to 400% or on a narrow viewport must scroll in two directions to read ordinary content, which is disorienting and, for some motor-impairment users, close to unusable. |
| Normative rule | At a 320 CSS-px-equivalent viewport width (Profile C) or 400% browser zoom, equivalent to a 320 CSS-px-wide viewport (Profile A), content reflows to a single column with no loss of information and no requirement to scroll in two dimensions for the page as a whole. This is the 400%/320-CSS-px reflow threshold of SC 1.4.10, distinct from `A11Y-PLATFORM-019`'s 200% resize threshold of SC 1.4.4 — the two obligations are checked independently and neither substitutes for the other. A `CMP-PLATFORM-006` `table` variant degrades to the `reading-list` shape at the largest text sizes rather than scrolling horizontally, per its own **Long content** rule; its own bounded, contained horizontal scroll remains the legitimate exception this obligation does not prohibit — see `A11Y-PLATFORM-036`. |
...
| Field | Value |
|---|---|
| Purpose | State the accessible-authentication obligation at the provider-neutral level this product can commit to now — as an affirmative no-memorization/no-forced-transcription rule, not as a permission derived from an OTP-specific exception — without selecting a vendor or inventing a mechanism `Q-OPS-001` has not yet resolved. |
...
| Requirement source | WCAG 2.2 SC 3.3.8 Accessible Authentication (Minimum); `accessibility/wcag-checklist.md`; `accessibility/cognitive.md` |
| User impact | A user who cannot memorize or manually transcribe a code under time pressure — including but not limited to users with cognitive, memory, or attention disabilities — is blocked from authenticating at all if no accessible alternative exists. |
| Normative rule | Authentication must not require memorization or forced manual transcription where paste, autofill, password-manager support, or another accessible mechanism avoids that cognitive burden, per `accessibility/wcag-checklist.md` SC 3.3.8 and `accessibility/cognitive.md`'s Accessible Authentication rule ("never require recalling/transcribing codes, solving puzzles, or re-entering info already provided"). This is not a blanket permission for OTP/code-entry merely because WCAG's authentication criterion carries a narrow object-recognition/received-object exception — the obligation is the affirmative one stated above, and it binds regardless of which specific challenge mechanism `Q-OPS-001` eventually selects. Concretely: the code can be pasted or auto-filled rather than requiring manual digit-by-digit transcription under time pressure, wherever the platform supports it; the throttle and expiry windows are stated plainly (`ERR-IDENTITY-003`, `-004`) rather than silently blocking; and password managers and platform auto-fill are never blocked. |
