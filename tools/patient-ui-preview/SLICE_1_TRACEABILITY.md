...
`docs/ux/05-build/IMPLEMENTATION_CONTRACTS.md` / `DESIGN_TRACEABILITY.md`. Not re-derived from a
prompt's conceptual screen list.

The mandatory Slice 1 booking chain remains unchanged. The requested discovery/decision follow-up
implemented canonical optional `SCR-ELIG-004` and `WGT-ELIG-002`; those additions are recorded here
because this file describes the current preview surface, not because the canonical slice taxonomy was
redefined.

## Flows implemented

`FLOW-IDENTITY-001` → `FLOW-CATALOG-001` → `FLOW-ELIG-001` → `FLOW-BOOKING-001` (mandatory chain),
plus the optional patient-safe eligibility explanation branch from `SCR-ELIG-003`.
...
| `SCR-ELIG-004` | `src/screens/EligibilityExplanationScreen.tsx` | `Patient/Screens/SCR-ELIG-004 Eligibility explanation` |
...
| `WGT-ELIG-002` (eligibility decision block, Patient variant) | `src/widgets/EligibilityDecisionBlock.tsx` | `SCR-ELIG-004` |
...
## Deferred beyond the current Patient preview (documented scope decisions, not gaps)
...
- A universal provider profile, service-less provider search, public raw S/P/H/I/risk screens, and
  a public provider-review feed are not invented. The current canonical discovery contracts remain
  service-and-branch scoped and expose only the Patient-safe projections they define.
