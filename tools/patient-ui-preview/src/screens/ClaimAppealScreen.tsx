...
import {
  type ClaimAppealDraft,
  type ClaimAppealProjection,
...
export interface ClaimAppealEvidenceSummary {
  id: string;
  label: string;
}
...
function OriginalDecisionCard({ claim }: { claim: PatientClaimDetail }) {
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: space('inline-sm') }}>
        <View style={{ flex: 1, gap: space('stack-xs') }}>
          <Helper>القرار الأصلي — يبقى محفوظًا</Helper>
...
function ExistingAppealCard({ appeal }: { appeal: ClaimAppealProjection }) {
  return (
    <View
    ...
    >
      ...
      {appeal.grounds ? (
        <View style={{ gap: space('stack-xs') }}>
          <BodyStrong>الأساس المسجّل</BodyStrong>
          <Body>{appeal.grounds}</Body>
        </View>
      ) : (
        <Body>يوجد اعتراض مسجّل لهذا القرار، لذلك لا نعرض نموذجًا ثانيًا.</Body>
      )}
...
export function ClaimAppealScreen({
  ...
  submitState = 'editing',
  subject = 'الاعتراض على قرار المطالبة',
  authority,
  initialGrounds = '',
  supportingEvidence = [],
  onSubmit,
  ...
  submitState?: ClaimAppealSubmitState;
  subject?: string;
  authority?: string;
  initialGrounds?: string;
  supportingEvidence?: ClaimAppealEvidenceSummary[];
  onSubmit: (draft: ClaimAppealDraft) => void;
  onBack: () => void;
}) {
  ...
  return (
    <Screen footer={<ActionBar actions={actions} />}>
      <Stack gap="stack-lg">
        <ScreenHeader
          eyebrow="اعتراض على قرار مطالبة"
          title="اعترض على القرار نفسه دون محو تاريخه"
          description="نافذة الاعتراض تأتي من لقطة السياسة التي حكمت القرار الأصلي، لا من إعدادات اليوم. يظل القرار وسببه ظاهرين طوال المسار."
        />
        <SubjectContextHeader subject={subject} authority={authority} />

        {decisionAvailable ? <OriginalDecisionCard claim={claim} /> : (
          <BlockedCard
            title="لا يمكن بدء الاعتراض قبل قراءة القرار الأصلي."
            body="لا نعرض نموذجًا ضد قرار لا يمكن قراءته مع سببه. عد إلى المطالبة وأعد تحميل السجل أولًا."
          />
        )}
        ...
        {decisionAvailable ? (
          ...
          >
            <Helper>لقطة السياسة الحاكمة للاعتراض</Helper>
            <BodyStrong>{claim.governingSnapshotLabel}</BodyStrong>
            <Body>يُتحقق من نافذة الاعتراض والشروط بالنسخة التاريخية المرتبطة بالقرار، ولا نبدّلها بسياسة أحدث.</Body>
          </View>
        ) : null}
        ...
        {decisionAvailable && deadlineIso ? (
          <DeadlineIndicator
            ...
            obligation="مهلة الاعتراض على هذا القرار"
            nowIso={CLAIMS_NOW_ISO}
            state={deadlineExpired ? 'lapsed' : 'running'}
          />
        ) : null}

        {appeal ? <ExistingAppealCard appeal={appeal} /> : null}

        {!appeal && decisionAvailable && !policyAllows ? (
          <BlockedCard
            title="لا تمنح لقطة السياسة لهذا القرار حق اعتراض من حسابك."
            body="يبقى القرار وسببه ظاهرين، لكننا لا نعرض نموذج إرسال عندما لا تسمح السياسة التاريخية بهذا الإجراء."
          />
        ) : null}

        {!appeal && decisionAvailable && policyAllows && !actorAuthorized ? (
          <BlockedCard
            title="لا تملك صلاحية تقديم هذا الاعتراض."
            body="يستطيع الطرف المخوّل في الحالة، أو وليّه ضمن نطاق تمثيل فعّال، تقديم الاعتراض. تغيير المريض المعروض لا ينشئ الصلاحية."
          />
        ) : null}

        {!appeal && decisionAvailable && policyAllows && actorAuthorized && deadlineExpired ? (
          <BlockedCard
            title="انتهت مهلة الاعتراض."
            body="هذه النافذة التاريخية انتهت ولا تُعامل كفشل قابل لإعادة المحاولة أو كسبب لإنشاء نية إرسال جديدة."
          />
        ) : null}
          ...
          <View style={{ gap: space('stack-lg') }}>
            {submitState === 'retryable-failure' ? (
              <BlockedCard
                ...
                body="احتفظنا بما كتبته. إعادة الإرسال تستخدم محاولة الإرسال نفسها بدل إنشاء اعتراض ثانٍ."
              />
            ) : null}
            ...
            <ValidationField
              ...
              helper="اشرح لماذا تعتقد أن القرار يحتاج مراجعة وفق الوقائع والسياسة التي حكمته. لا يعيد هذا الحقل كتابة القرار الأصلي."
            ...
            >
              ...
              {supportingEvidence.length > 0 ? supportingEvidence.map((item) => (
                <Body key={item.id}>{item.label}</Body>
              )) : (
                <Helper>لا توجد مستندات مرتبطة بالاعتراض. يمكن إرسال الأساس النصي وحده عندما تسمح السياسة بذلك.</Helper>
              )}
            ...
            >
              <BodyStrong>ما الذي لا يفعله الاعتراض؟</BodyStrong>
              <Body>لا يمحو القرار الأصلي، ولا يضمن نتيجة أو مبلغًا ماليًا، ولا ينفّذ أي حركة أموال داخل UberTib.</Body>
            </View>
          </View>
        ) : null}
      </Stack>
